...
let
  ...
  node =
    lock.nodes.${name}
      or (builtins.throw "getInputVersionFromLock: input not found in flake.lock: ${name}");
  ref =
    if node ? original && node.original ? ref then
      node.original.ref
    else
      builtins.throw "getInputVersionFromLock: original.ref not found for input: ${name}";
in
if prefix == null then ref else lib.strings.removePrefix prefix ref
